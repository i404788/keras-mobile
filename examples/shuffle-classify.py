...
from keras.layers import Input, Dense, Conv2D, MaxPooling2D, UpSampling2D, Dropout, Flatten,ReLU
...
def get_model():
    ...
    for i in range(4):
        x = ShuffleBasic(32//(2 ** i), hp_bottleneck_size)(x)
        x = Dropout(0.1)(x)
        x = ShuffleStride(32//(2 ** i), hp_bottleneck_size)(x)
        x = ReLU()(x)
